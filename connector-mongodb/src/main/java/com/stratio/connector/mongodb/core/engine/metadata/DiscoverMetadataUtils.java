...
public final class DiscoverMetadataUtils {
    ...
    public static Map<String, String> discoverFieldsWithType(DBCollection collection, String sample_probability) {
        ...
        String map = "function() {  if(Math.random() <= sample_number) {for (var key in this) {var type = typeof(this[key]); if(type == \"object\"  || !type){type = \"native\";};emit(key, type);}} } ";
        String reduce = "function(key, values) { var result = \"\"; for (var i = 0; i < values.length; i++){ var v = values[i];if(v == \"native\"){result = \"native\"; break;}if(v == \"string\"){result = \"string\"; break;} if(v == \"number\"){result = \"number\"} if(v == \"boolean\" && result == \"number\"){result = \"string\"; break;}if(v == \"number\" && result == \"boolean\"){result = \"string\"; break;} if(v==\"boolean\"){result = \"boolean\"}};return result; }";
